...
public class CharacterSheetInteraction {
				...
				.then(Commands.argument("statted", StattedArgumentType.CHARACTER_NULLABLE)
					.then(Commands.argument("page", StatPageArgumentType.ARGUMENT)
						.executes((context) -> {
							...
							switchPage(e, editAction, editBuilder, statted, context.getArgument("page", StatEmbedPage.class));
							editAction.queue();
							return 1;
						})
					)
	...
	private static MessageEditCallbackAction switchPage(ComponentInteraction i, MessageEditCallbackAction e, MessageEditBuilder b, Statted statted, StatEmbedPage page) {
		return e.setEmbeds(new StatEmbedBuilder(statted, page, b).getEmbed()).setComponents(new StatInteractionBuilder(statted, page).getComponents());
	}
