...
public class ChoiceArgument<T> implements ArgumentType<T>, Choice<T> {
	...
	public <S> T parse(S context, StringReader reader) throws CommandSyntaxException {
		String val = Commands.readString(reader);
		if(type == String.class) {
			...
			return (T) Commands.readString(reader);
		}
			...
			return (T)(Integer) Integer.parseInt(Commands.readString(reader));
		}
		else if (type == Double.class) {
			return (T)(Double) Double.parseDouble(Commands.readString(reader));
		}
		else {
